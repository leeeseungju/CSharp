...
public class Player : MonoBehaviour
{
    ...
    void Turn()
    {
        ...
        //#2. 마우스에 의한 회전
        if(fDown && !isDead) //if 없으면 평상시에도 마우스 방향 바라봄, 총 발사할때만 마우스 방향 바라보게
        {
            ...
            if (Physics.Raycast(ray, out rayHit, 100))
            {
                ...
                nextVec.y = 0; //벽이나 물체 바라보면 플레이어 몸이 기울어지는 것 수정
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.tag == "EnemyBullet")
        {
            if (!isDamage)
            {
                ...
                StartCoroutine(OndDamage(isBossAttack));
            }
    ...
    IEnumerator OndDamage(bool isBossAttack)
    {
        isDamage = true;
